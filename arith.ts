import { parseArith } from "npm:tiny-ts-parser";

type Type =
| { tag: "Boolean" }
| { tag: "Number" }

type Term =
| { tag: "true" }
| { tag: "false" }
| { tag: "if"; cond: Term; thn: Term; els: Term }
| { tag: "number"; n: number }
| { tag: "add"; left: Term; right: Term }


function typecheck(t: Term): Type {
  switch (t.tag) {
    case "true":
      return { tag: "Boolean" };
    case "false":
      return { tag: "Boolean" };
    case "if": {
      const _ = typecheck(t.cond);
      //if (condTy.tag !== "Boolean" && condTy.tag !== "Number") throw "wrong type in condition";
      const thnTy = typecheck(t.thn);
      const elsTy = typecheck(t.els);
      if (thnTy.tag !== elsTy.tag) {
        throw "then and else have different types";
      }
      return thnTy;
    }
    case "number":
      return { tag: "Number" };
    case "add": {
      const leftTy = typecheck(t.left);
      if (leftTy.tag !== "Number") throw "number expected";
      const rightTy = typecheck(t.right);
      if (rightTy.tag !== "Number") throw "number expected";
      return { tag: "Number" };
    }
  }
}

console.log(typecheck(parseArith("1 + true ? true : false")));

//演習問題
//条件演算子の条件式が任意の型の値でもOKにするように変更
//基本的にはconfの型を確認するのをやめれば良いのですが、typecheck(t.connd)を丸ごと削除すると問題が起きる
　//BooleanとNumber型意外の型を保持しない設計になっているのに、それ以外の型が入ってもエラーが起きない

//模範解答
  // (1+true) ? true: false のようなプログラムがOKになる
  //その部分の返り値を使わないとしてもtypecheck自体は行う必要がある
